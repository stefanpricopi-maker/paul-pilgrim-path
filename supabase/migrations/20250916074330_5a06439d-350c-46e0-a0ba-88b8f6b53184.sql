-- Fix Critical RLS Recursion Issue - Part 2: Update RLS Policies
...
WITH CHECK (
  id = auth.uid() AND 
  -- Prevent users from changing their own admin status unless they are admin
  (OLD.is_admin IS NOT DISTINCT FROM NEW.is_admin OR public.is_current_user_admin())
);
